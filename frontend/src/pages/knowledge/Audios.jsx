        ...
        <div className="page-enter">
            ...
            <div className="km-toolbar">
                <div className="km-toolbar-left">
                    <label className="btn btn-primary km-upload-btn" style={{ marginLeft: -4, borderRadius: 6, cursor: uploading ? 'not-allowed' : 'pointer' }}>
                ...
                <div className="km-toolbar-right">
                    <div className="km-search">
                        ...
                        <input type="text" placeholder="搜索音频..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                    </div>
            ...
            <div style={{ padding: '0 0 12px', fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)' }}>
                {filtered.length} 个音频 · 通过钉钉 OpenClaw 接收的音频文件（mp3/wav）
            </div>

            {loading ? (
                <div style={{ textAlign: 'center', padding: 60, color: 'var(--color-text-muted)' }}>加载中...</div>
            ) : filtered.length === 0 ? (
                <EmptyState icon="🎵" title="暂无音频" desc="通过钉钉 OpenClaw 发送 mp3、wav 格式文件后将显示在这里" />
            ) : viewMode === 'list' ? (
                <div className="km-table-wrap">
                    <table className="km-table">
                        ...
                        <tbody>
                            {filtered.map(a => (
                                <tr key={a.id} className="km-row" onClick={() => navigate(`/knowledge/documents/${a.id}`)}>
                                    <td className="km-td-name">
                                        <img src="/icons/MP3图标.png" alt="音频" className="km-file-icon-img" style={{ width: 24, height: 24, marginRight: 8, verticalAlign: 'middle' }} />
                                        <span className="km-file-title">{a.title}</span>
                                    </td>
                                    <td className="km-td-size">{formatSize(a.file_size)}</td>
                                    <td className="km-td-date">{formatDate(a.created_at)}</td>
                                    <td onClick={e => e.stopPropagation()}>
                                        <button className="km-icon-btn" title="删除" onClick={() => handleDelete(a.id)}>
                ...
                <div className="file-grid">
                    {filtered.map(a => (
                        <div key={a.id} className="card file-card" onClick={() => navigate(`/knowledge/documents/${a.id}`)}>
                            <div className="file-icon" style={{ background: 'transparent' }}>
                                <img src="/icons/MP3图标.png" alt="音频" style={{ width: 48, height: 48 }} />
                            </div>
                            <div className="file-name" title={a.title}>{a.title}</div>
                            <div className="file-meta">{formatSize(a.file_size)} · {formatDate(a.created_at)}</div>
                        </div>
                    ))}
                </div>
            )}
