...
export default function Notes() {
    ...
    const loadNotes = () => {
        ...
        docsApi.list({ doc_type: 'note' })
            .then(async (d) => {
                ...
                // Load content for all notes for the rich text display
    ...
    return (
        <div className="page-enter">
                    ...
                    {filtered.map(n => {
                        ...
                        return (
                            <div
                                key={n.id}
                                className="card"
                                style={{ padding: '16px 20px', cursor: 'pointer', borderLeft: `4px solid #4285F4`, transition: 'all 0.2s' }}
                                ...
                                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        {isExpanded ? (
                                            ...
                                            <div style={{ color: 'var(--color-text-muted)', lineHeight: 1.5, fontSize: 'var(--font-size-sm)' }}>
                                                {preview || '(无内容)'}
                                            </div>
                                        )}
                                    </div>
            ...
            {/* Create Note Modal */}
            <Modal open={createOpen} title="新建富文本笔记" onClose={() => setCreateOpen(false)} footer={
                <>
                ...
                <div className="form-group">
                    ...
                    <input className="form-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="笔记标题" />
                </div>
                <div className="form-group" style={{ marginBottom: 40 }}>
                    <label className="form-label">内容</label>
                    <SimpleEditor value={content} onChange={setContent} style={{ height: 250 }} placeholder="开始记录富文本笔记..." />
                </div>
            </Modal>

            {/* Edit Note Modal */}
            <Modal open={!!editNoteId} title="编辑笔记" onClose={() => setEditNoteId(null)} footer={
                <>
                    <button className="btn btn-outline" onClick={() => setEditNoteId(null)}>取消</button>
                    <button className="btn btn-primary" onClick={handleEditSave}>更新</button>
                </>
            }>
                <div className="form-group">
                    <label className="form-label">标题</label>
                    <input className="form-input" value={editTitle} onChange={e => setEditTitle(e.target.value)} placeholder="笔记标题" />
                </div>
                <div className="form-group" style={{ marginBottom: 40 }}>
                    <label className="form-label">内容</label>
                    <SimpleEditor value={editContent} onChange={setEditContent} style={{ height: 250 }} placeholder="修改笔记内容..." />
                </div>
            </Modal>
        </div>
    );
